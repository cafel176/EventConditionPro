...
var EventConditionPro_GetEventName = function (event) {
    ...
    if (!('_eventId' in event)) {
        return "公共事件 " + $dataCommonEvents[event._commonEventId].name
    }
    return "事件 " + $dataMap.events[event._eventId].name
}
...
var EventConditionPro_ProcessPluginCommand = function (event, outer, eventItem) {
    ...
    // 条件
        ...
        for (let i = 0; i < conditions.length; i++) {
            ...
            let resultName = condition.name;
            if (resultName === null || String(resultName) === "undefined")
                resultName = ""
            let checkTarget1Name = condition.checkTarget1Name;
            if (checkTarget1Name === null || String(checkTarget1Name) === "undefined")
                checkTarget1Name = ""
    ...
    // 运算
        ...
        for (let i = 0; i < operates.length; i++) {
            ...
            const value1 = EventConditionPro_GetSwitchValue(event, outer, operate.checkTarget1)
            const value2 = EventConditionPro_GetSwitchValue(event, outer, operate.checkTarget2)
...
DataManager.onLoad = function (object) {
    ...
    if (EventConditionPro_loadDataCommonEvents) {
        for (let i = $dataCommonEvents.length - 1; i >= 0; --i) {
            ...
            // 对事件页所有的插件指令进行处理
            EventConditionPro_Load($dataCommonEvents[i], $dataCommonEvents[i]);
        }
...
Game_Event.prototype.setupPageSettings = function () {
    ...
    const Triggers = EventConditionPro_GetTriggers(page, 4)
...
Game_Map.prototype.setupAutorunCommonEvent = function () {
    for (const commonEvent of this.autorunCommonEvents()) {
        ...
        const Triggers = EventConditionPro_GetTriggers(commonEvent, 1)
        // 有触发
        if (Triggers && Triggers.length > 0) {
            // 判断触发
            let Check = true
            for (let i = 0; i < Triggers.length; ++i) {
                if (!EventConditionPro_ProcessPluginCommand(commonEvent, commonEvent, Triggers[i])) {
                    Check = false
                    break
                }
            }
...
Game_CommonEvent.prototype.isActive = function () {
    ...
    if (event.trigger === 2) {
        ...
        const Triggers = EventConditionPro_GetTriggers(event, 2)
        // 有触发
        if (Triggers && Triggers.length > 0) {
            // 判断触发
            let Check = true
