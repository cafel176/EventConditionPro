 ...
 * 
 * 12. 设置环境(用于事件)：事件页内执行运算时，在将脚本用于条件分歧前需要执行本指令
 * 
 ...
 * @param name
 * @text 结果名称
 * @desc 本条件的比较结果会保存在这么一个名字的临时变量里以供取用，留空为不保存
 * 
 * @param negative
 * @text 取反
 * @desc 对当前条件的结果取反
 ...
 * @text 比较对象1名称
 * @desc 比较对象1的值会保存在这么一个名字的临时变量里以供取用，留空为不保存
 * 
 * @param checkTarget1
 * @text 比较对象1
...
var EventConditionPro_ProcessPluginCommand = function (event, page, eventItem) {
    if (!eventItem) {
        console.log($dataMap.events[event._eventId].name + " " + "条件为空，跳过")
        return null
    }
    ...
    // 条件
        ...
        for (let i = 0; i < conditions.length; i++) {
            ...
            // 事件位置X
                ...
                const checkTarget1 = EventConditionPro_GetIndex(condition.checkTarget1);
                const value = EventConditionPro_GetVariableValue(event, page, condition.checkTarget2)
    ...
    // 输出
        ...
        console.log($dataMap.events[event._eventId].name + " " + name + "：" + EventConditionPro_GetTempValue(page, name))
        return true
    }
    // 提交结果
    ...
    console.log($dataMap.events[event._eventId].name + " " + "未识别的指令，跳过：" + commandName)
    return null
}
